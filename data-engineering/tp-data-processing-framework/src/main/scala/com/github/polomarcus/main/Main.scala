...
object Main {
  def main(args: Array[String]) {
    ...
    // Tips: use a groupBy
    val df_nb_subject_groupby=enrichedDataset.groupBy("containsWordGlobalWarming")count()
    df_nb_subject_groupby.show()
    ...
    val df_nb_subject_query = spark.sql("SELECT containsWordGlobalWarming, COUNT(*) FROM news GROUP BY containsWordGlobalWarming")
    df_nb_subject_query.show()
